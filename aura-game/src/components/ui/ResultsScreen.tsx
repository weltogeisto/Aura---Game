      ...
      <div className="w-full max-w-6xl max-h-[92dvh] overflow-y-auto rounded-2xl border border-white/10 bg-gray-950/95 p-4 shadow-2xl sm:p-6 md:p-8 xl:grid xl:grid-cols-[1.7fr_1fr] xl:gap-6">
        ...
        <aside className="mt-6 space-y-4 xl:mt-0 xl:sticky xl:top-4 xl:self-start">
          <div className="rounded-xl border border-orange-500/35 bg-black/35 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-orange-200">{UI_COPY_MAP.results.operationHeading}</p>
            <p className="mt-2 text-sm text-gray-300">{UI_COPY_MAP.results.operationBody}</p>
          </div>

          <div className="rounded-xl border border-white/10 bg-gray-900/80 p-4 text-sm text-gray-300">
            <p className="text-xs uppercase tracking-[0.2em] text-gray-300">Run facts</p>
            <p className="mt-2">Target: {lastShotResult.hitTargetName || 'No registered target'}</p>
            <p>Impact: {lastShotResult.hitLocationLabel}</p>
            <p>Score: {formatCurrency(totalScore)}</p>
            <p>Result type: {isDadaistHit ? 'Dadaist anomaly' : 'Canonical hit'}</p>
          </div>

          <div className="rounded-xl border border-white/10 bg-black/35 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-orange-200">Next loop checklist</p>
