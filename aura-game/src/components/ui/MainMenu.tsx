...
export function MainMenu() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-gray-950 via-gray-900 to-black px-4">
      <div className="w-full max-w-2xl max-h-[calc(100vh-2rem)] overflow-y-auto rounded-2xl border border-white/10 bg-black/35 p-6 text-center shadow-2xl backdrop-blur md:p-10">
        <p className="text-xs uppercase tracking-[0.32em] text-orange-300/85">{UI_COPY_MAP.release.badge}</p>
        <h1 className="mt-3 text-4xl sm:text-5xl font-bold text-transparent bg-gradient-to-r from-red-500 to-orange-400 bg-clip-text md:text-6xl">
        ...
        <button
          ...
          className="mt-6 md:mt-10 rounded-full bg-red-600 px-9 py-3 text-lg font-semibold text-white transition-colors hover:bg-red-700"
        >
          {UI_COPY_MAP.mainMenu.ctaScenarioSelect}
        </button>

        <p className="mt-4 md:mt-10 text-xs leading-relaxed text-gray-500 md:text-sm">{UI_COPY_MAP.mainMenu.criticFraming}</p>
      </div>
    </div>
  );
}
